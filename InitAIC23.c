...
void InitAIC23_16bit(void);
void InitAIC23_16bit_32kHz(void);
void InitAIC23_16bit_8kHz(void);
...
void InitAIC23_16bit(void) {
    ...
    // I2S
    command = I2Sdigaudinterface_16bit(); // AIC23 master mode, I2S mode,16-bit data, LRP=1 to match with XDATADLY=1
    SpiTransmit(command);
    SmallDelay();
    command = CLKsampleratecontrol (SR48);
    SpiTransmit(command);
